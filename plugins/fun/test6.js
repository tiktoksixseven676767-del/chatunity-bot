...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    // Controllo se l'utente ha inserito un testo
    ...
    try {
        ...
        // Invia la miniatura con le info
        await conn.sendMessage(m.chat, { 
            image: { url: thumbnail }, 
            caption: caption 
        }, { quoted: m });

        // Download Audio tramite API (Sostituibile se hai un convertitore locale)
        ...
        console.error(e);
        m.reply('❗ Errore: Non è stato possibile recuperare l\'audio. Riprova tra poco.');
        await conn.sendMessage(m.chat, { react: { text: '❌', key: m.key } });
    }
};
...
handler.command = /^(play1|yt|youtube)$/i;
